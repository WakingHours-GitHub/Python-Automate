...
def getVerificationCode(driver):
    ...
    # 自定义降噪 -> 原理不明白
    kernel = np.array([[0, -1, 0], [-1, 4.85, -1], [0, -1, 0]], np.float32)
    cv2img = cv2.filter2D(cv2img, -1, kernel=kernel)
...
def login(user, driver):
    while True:
        ...
        try:
            ...
            driver.find_element(By.XPATH,"/html/body/div[11]/div/table/tbody/tr[2]/td/div/div/div[1]/input").send_keys(user["username"])
            driver.find_element(By.XPATH,"/html/body/div[11]/div/table/tbody/tr[2]/td/div/div/div[2]/input").send_keys(user["password"])
            driver.find_element(By.XPATH, "/html/body/div[11]/div/table/tbody/tr[2]/td/div/div/div[3]/input").send_keys(str(getVerificationCode(driver)))
            break
        except:
            "登录界面找到"
